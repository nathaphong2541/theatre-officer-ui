...
import { UnionService } from 'src/app/modules-admin/layout/services/master/unions-guilds-memberships.service';

@Component({
  selector: 'app-list-union',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, FormsModule],
  templateUrl: './list-union.component.html',
  styleUrl: './list-union.component.css'
})
export class ListUnionComponent implements OnInit {
  loading = false;
  error = '';
  ...
  load(): void {
    ...
    this.unionService.list(this.page, this.size).subscribe({
      next: (res: any) => {
      ...
      error: (err) => {
        this.loading = false;
        this.error = err?.message ?? 'Load unions failed';
      },
    });
  }
  ...
  save(): void {
    ...
    if (this.mode === 'create') {
      this.unionService.create(payload).subscribe({
        next: () => {
          this.loading = false;
          this.closeModal();
          this.load();
    ...
    if (!this.editingId) {
      this.loading = false;
      this.error = 'Missing union id';
      return;
    }

    this.unionService.update(this.editingId, payload).subscribe({
      next: () => {
        this.loading = false;
        this.closeModal();
        this.load();
  ...
  remove(item: BaseMaster): void {
    ...
    const ok = confirm(`ยืนยันลบ Union: ${this.getNameTh(item) || this.getNameEn(item) || id} ?`);
    if (!ok) return;

    this.loading = true;
    this.error = '';

    this.unionService.delete(id).subscribe({
      next: () => {
        this.loading = false;
  ...
  getId(x: BaseMaster): number {
    return (x as any).id ?? 0;
  }
  getNameEn(x: BaseMaster): string {
    return (x as any).nameEn ?? '';
  }
  getNameTh(x: BaseMaster): string {
    return (x as any).nameTh ?? '';
  }
  getDesc(x: BaseMaster): string {
    return (x as any).description ?? '';
  }
