...
import { PartnerDirectoryService } from 'src/app/modules-admin/layout/services/master/partner-directories.service';

@Component({
  selector: 'app-list-partner-directory',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, FormsModule],
  templateUrl: './list-partner-directory.component.html',
  styleUrl: './list-partner-directory.component.css'
})
export class ListPartnerDirectoryComponent implements OnInit {
  loading = false;
  error = '';
  ...
  constructor(private fb: FormBuilder, private service: PartnerDirectoryService) { }
  ...
  load(): void {
    ...
    this.service.list(this.page, this.size).subscribe({
      ...
      error: (err) => {
        this.loading = false;
        this.error = err?.message ?? 'Load partner directories failed';
      },
    });
  }
  ...
  remove(item: BaseMaster): void {
    ...
    const ok = confirm(`ยืนยันลบ Partner Directory: ${this.getNameTh(item) || this.getNameEn(item) || id} ?`);
    if (!ok) return;
