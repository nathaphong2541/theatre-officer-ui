...
import { ExperienceLevelService } from 'src/app/modules-admin/layout/services/master/experience-levels.service';
import { BaseMaster } from 'src/app/modules-admin/layout/services/master/model/base';

@Component({
  selector: 'app-list-experience-level',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, FormsModule],
  templateUrl: './list-experience-level.component.html',
  styleUrl: './list-experience-level.component.css'
})
export class ListExperienceLevelComponent implements OnInit {
  loading = false;
  error = '';
  ...
  constructor(private fb: FormBuilder, private service: ExperienceLevelService) { }
  ...
  load(): void {
    ...
    this.service.list(this.page, this.size).subscribe({
      ...
      error: (err) => {
        this.loading = false;
        this.error = err?.message ?? 'Load experience levels failed';
      },
    });
  }
  ...
  remove(item: BaseMaster): void {
    ...
    const ok = confirm(`ยืนยันลบ Experience Level: ${this.getNameTh(item) || this.getNameEn(item) || id} ?`);
    if (!ok) return;
