...
import { GenderIdentityService } from 'src/app/modules-admin/layout/services/master/gender-identity.service';

@Component({
  selector: 'app-list-gender-identity',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, FormsModule],
  templateUrl: './list-gender-identity.component.html',
  styleUrl: './list-gender-identity.component.css'
})
export class ListGenderIdentityComponent implements OnInit {
  loading = false;
  error = '';
  ...
  constructor(private fb: FormBuilder, private service: GenderIdentityService) { }
  ...
  load(): void {
    ...
    this.service.list(this.page, this.size).subscribe({
      ...
      error: (err) => {
        this.loading = false;
        this.error = err?.message ?? 'Load gender identities failed';
      },
    });
  }
  ...
  remove(item: BaseMaster): void {
    ...
    const ok = confirm(`ยืนยันลบ Gender Identity: ${this.getNameTh(item) || this.getNameEn(item) || id} ?`);
    if (!ok) return;
