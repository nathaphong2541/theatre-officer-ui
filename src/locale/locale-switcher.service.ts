...
export class LocaleSwitcherService {
    ...
    /** อ่าน locale จาก segment แรกของพาธ (default = th) */
    currentLocale(): 'th' | 'en' {
    ...
    switchTo(locale: 'th' | 'en') {
        const inner = this.stripBase(this.doc.location.pathname);
        const segs = inner.split('/').filter(Boolean); // ตัวอย่าง: ['th','about'] หรือ ['en','products', '123']

        if (segs.length === 0) segs.push(locale);
        else segs[0] = locale; // แทนที่ segment แรกเป็นภาษาที่เลือก
    ...
    /**
     * (ออปชัน) บังคับให้เริ่มที่ /th ถ้ายังไม่มี segment ภาษา
     * เรียก method นี้ครั้งเดียวตอนบูตแอป (เช่นใน APP_INITIALIZER หรือ AppComponent.ngOnInit)
     */
    ensureDefaultThai() {
        const inner = this.stripBase(this.doc.location.pathname);
